...
init_toBytes();
init_keccak256();
var DEPOSIT_EVENT_SIG = keccak256(toBytes("Deposit(address,uint256)"));
var WITHDRAWAL_EVENT_SIG = keccak256(toBytes("Withdrawal(address,uint256)"));
var PAUSE_EVENT_SIG = keccak256(toBytes("EmergencyPause(address,uint256)"));
...
function toBase64(str) {
  ...
  for (let i2 = 0;i2 < len2; i2 += 3) {
    ...
    const b = i2 + 1 < len2 ? str.charCodeAt(i2 + 1) : 0;
    const c = i2 + 2 < len2 ? str.charCodeAt(i2 + 2) : 0;
    const triplet = a << 16 | b << 8 | c;
    result += B64_CHARS[triplet >> 18 & 63];
    result += B64_CHARS[triplet >> 12 & 63];
    result += i2 + 1 < len2 ? B64_CHARS[triplet >> 6 & 63] : "=";
    result += i2 + 2 < len2 ? B64_CHARS[triplet & 63] : "=";
  }
  return result;
}
function protoBlockNumber(pb) {
  if (!pb || !pb.absVal || pb.absVal.length === 0)
    return "0";
  let hex = "";
  for (let i2 = 0;i2 < pb.absVal.length; i2++) {
    hex += pb.absVal[i2].toString(16).padStart(2, "0");
  }
  const val = BigInt("0x" + hex);
  return (pb.sign && pb.sign < 0n ? -val : val).toString();
}
var httpClient = new ClientCapability2;
function sendWebhookNotification(runtime2, payload) {
  const config = runtime2.config;
  const sendNotification = httpClient.sendRequest(runtime2, (sendRequester) => {
    ...
    const isOk = ok(result2);
    const responseText = text(result2);
    return {
      success: isOk,
  ...
  runtime2.log(`  Webhook response: status=${result.status}, success=${result.success}`);
}
function analyzeAmount(amountWei, thresholdWei) {
...
var onDepositDetected = (runtime2, log) => {
  const config = runtime2.config;
  const contractAddress = bytesToHex(log.address);
  const txHash = bytesToHex(log.txHash);
  const blockNumber = protoBlockNumber(log.blockNumber);
  runtime2.log(`[SentinelDAO CRE] Deposit event detected on vault ${contractAddress}`);
  runtime2.log(`  Block: ${blockNumber} | Tx: ${txHash}`);
  const amountHex = bytesToHex(log.data);
  const amountWei = BigInt(amountHex);
  const amountEth = formatUnits(amountWei, 18);
  runtime2.log(`  Amount: ${amountEth} ETH (${amountWei} wei)`);
  let depositor = "unknown";
  if (log.topics.length >= 2) {
    depositor = bytesToHex(log.topics[1].slice(12));
  }
  runtime2.log(`  From: ${depositor}`);
  const { severity, threatType, isLarge } = analyzeAmount(amountWei, config.thresholdWei);
  const thresholdEth = formatUnits(BigInt(config.thresholdWei), 18);
  if (isLarge) {
    runtime2.log(`  ⚠️  THREAT DETECTED: Deposit of ${amountEth} ETH exceeds ${thresholdEth} ETH threshold`);
    runtime2.log(`  Severity: ${severity.toUpperCase()}`);
  } else {
    runtime2.log(`  ✓ Normal deposit: ${amountEth} ETH (below ${thresholdEth} ETH threshold)`);
  }
  ...
  runtime2.log(`  Sending webhook notification to ${config.webhookUrl}...`);
  sendWebhookNotification(runtime2, webhookPayload);
  if (isLarge) {
    runtime2.log(`  → Action: Recommending emergency pause for vault ${contractAddress}`);
  }
  return `Deposit analyzed: ${amountEth} ETH from ${depositor} | Severity: ${severity}`;
};
var onWithdrawalDetected = (runtime2, log) => {
  const config = runtime2.config;
  const contractAddress = bytesToHex(log.address);
  const txHash = bytesToHex(log.txHash);
  const blockNumber = protoBlockNumber(log.blockNumber);
  runtime2.log(`[SentinelDAO CRE] Withdrawal event detected on vault ${contractAddress}`);
  runtime2.log(`  Block: ${blockNumber} | Tx: ${txHash}`);
  const amountHex = bytesToHex(log.data);
  const amountWei = BigInt(amountHex);
  const amountEth = formatUnits(amountWei, 18);
  ...
  runtime2.log(`  To: ${recipient}`);
  const { severity, threatType, isLarge } = analyzeAmount(amountWei, config.thresholdWei);
  const thresholdEth = formatUnits(BigInt(config.thresholdWei), 18);
  if (isLarge) {
    runtime2.log(`  ⚠️  THREAT DETECTED: Withdrawal of ${amountEth} ETH exceeds ${thresholdEth} ETH threshold`);
    runtime2.log(`  Severity: ${severity.toUpperCase()}`);
  } else {
    runtime2.log(`  ✓ Normal withdrawal: ${amountEth} ETH (below ${thresholdEth} ETH threshold)`);
  }
  const webhookPayload = {
    source: "cre-workflow",
    workflowName: "sentinel-defense",
  ...
  sendWebhookNotification(runtime2, webhookPayload);
  if (isLarge) {
    runtime2.log(`  → Action: Flagging large withdrawal from vault ${contractAddress}`);
  }
  return `Withdrawal analyzed: ${amountEth} ETH to ${recipient} | Severity: ${severity}`;
};
var onPauseDetected = (runtime2, log) => {
  const config = runtime2.config;
  const contractAddress = bytesToHex(log.address);
  const txHash = bytesToHex(log.txHash);
  const blockNumber = protoBlockNumber(log.blockNumber);
  ...
  runtime2.log(`[SentinelDAO CRE] EmergencyPause detected on vault ${contractAddress}`);
  runtime2.log(`  Triggered by: ${triggeredBy}`);
  runtime2.log(`  Tx: ${txHash} | Block: ${blockNumber}`);
  const webhookPayload = {
    source: "cre-workflow",
    workflowName: "sentinel-defense",
